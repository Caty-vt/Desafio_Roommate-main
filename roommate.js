...
const guardarUsuario = async (dataUsuario) => {
    try {
        const usuariosJSON = await JSON.parse(fs.readFileSync("file/roommates.json", "utf8"))
        usuariosJSON.roommates.push(dataUsuario);
        fs.writeFileSync("file/roommates.json", JSON.stringify(usuariosJSON, null, 4));
    } catch (err) {
...
const actualizarUsuario = async (gasto) => {
    try {
        let usuariosJSON = await JSON.parse(fs.readFileSync("file/roommates.json", "utf8"));
        let arregloUsuarios = usuariosJSON.roommates;

        let gastosJSON = await JSON.parse(fs.readFileSync("file/gastos.json", "utf8"));
